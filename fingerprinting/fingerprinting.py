...
def fingerprinting(wav_path: str) -> dict:
    """
    Receives the full path to a .wav file and returns
    the fingerprint_map: Dict[hash, List[(time, song_id)]]
    """
    if not os.path.isfile(wav_path):
        raise FileNotFoundError(f"File does not exist: {wav_path}")

    # Extracts only the filename to use as song_id
    ...
    for i, spectrum in enumerate(fft_applied):
        ...
        time = i * hop_ms / 1000
        peaks_per_window.append({'time': time, 'peaks': peaks})

    # Generate and return the fingerprint map
    return generate_fingerprint_map(peaks_per_window, song_id)
